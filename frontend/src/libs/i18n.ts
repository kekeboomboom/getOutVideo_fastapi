...
export default getRequestConfig(async ({ locale }) => {
  // Handle undefined locale by using the default locale
  const validLocale = locale && AllLocales.includes(locale) ? locale : AppConfig.defaultLocale;

  // Only call notFound if the locale is provided but invalid
